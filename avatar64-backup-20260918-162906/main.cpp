...
static const unsigned long RHEA_IDLE_TIMEOUT_MS = 20000UL;
    ...
    bool blink)
{
    ...
    // RHEA1-F5D: compact multicolor flower/sunburst face.
    // Petal/ray structure + face outline: red
    // Eyes and mouth: white
    const uint16_t petalColor =
        RED;

    const uint16_t faceColor =
        RED;

    const uint16_t featureColor =
        WHITE;
    ...
    // Row 3: face + horizontal rays.
    // Draw the red structure first.
    display.setCursor(x - 9, y);
    display.print("--   (       )   --");

    // Overlay eyes and mouth in white.
    display.setTextColor(
        featureColor,
        BLACK
    );
    ...
    // Use a simple ASCII smile that the default font renders reliably.
    display.setCursor(x + 45, y + 6);
    display.print("u");
    ...
    RecordInfo &record)
{
    ...
    static StaticJsonDocument<1792> json;
...
bool connectWifi()
{
    showNetworkStatus("Connecting Wi-Fi...");

    Serial.print(
        "Connecting Wi-Fi to SSID: "
    );
    Serial.println(RHEA_WIFI_SSID);
    ...
    const String &query)
{
        ...
        summary.matchedVariants)
    {
        ...
        Serial.print(
            " variant(s) have no value and are excluded."
        );
    }
    ...
    const ToolRequest &request)
{
    ...
    if (request.countProductTypePrefix.length() > 0)
    {
        ...
        if (request.countProductTypePrefix == "Pop!")
        {
            label += "Pops";
        }
        else
...
void runSearch()
{
        ...
        ToolKind::Unknown)
    {
        ...
        display.println("list, or update records.");
        display.println();
        display.println("[ENTER] Ask me something else");
